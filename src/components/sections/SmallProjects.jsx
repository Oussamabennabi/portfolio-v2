...
const BigProjects = () => {
	...
	useEffect(() => {
		if (expand) {
			...
			setProjects(smallProjects.slice(0, 8));
		}
	}, [expand, smallProjects]);

	return (
		<div className="transition-all">
			<div className="container flex flex-col justify-center items-center max-w-[90rem] mx-auto p-6">
				<h1 className="text-6xl text-center mb-10">Fun Projects.</h1>
				<div className=" transition-all grid md:grid-cols-2  grid-cols-1 gap-6  xl:grid-cols-4 ">

					{projects.map((data) => {
						return <SmallProjectCard key={data._id} project={data} />;
					})}
				</div>
				<button
				...
				>
					...
					<svg
					className={`${expand?"rotate-0":"rotate-180"} transition-all scale-125`}
						width="24"
						height="24"
						fill="none"
					>
